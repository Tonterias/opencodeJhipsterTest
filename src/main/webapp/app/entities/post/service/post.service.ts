...
type RestOf<T extends IPost | NewPost> = Omit<T, 'creationDate' | 'publicationDate'> & {
...
export class PostsService {
  ...
  protected convertValueFromServer(restPost: RestPost): IPost {
    return {
      ...
      publicationDate: restPost.publicationDate ? dayjs(restPost.publicationDate) : undefined,
    };
  }
}
...
export class PostService extends PostsService {
  ...
  protected convertValueFromClient<T extends IPost | NewPost | PartialUpdatePost>(post: T): RestOf<T> {
    return {
      ...
      publicationDate: post.publicationDate?.toJSON() ?? null,
    };
  }
