...
import { sampleWithRequiredData } from '../urllink.test-samples';
...
describe('Urllink Management Component', () => {
  ...
  let comp: Urllink;
  let fixture: ComponentFixture<Urllink>;
  ...
  beforeEach(() => {
    ...
    fixture = TestBed.createComponent(Urllink);
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: 16948 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();

    // THEN
    expect(comp.isLoading()).toEqual(false);
    expect(comp.urllinks()[0]).toEqual(expect.objectContaining({ id: 16948 }));
  });
  ...
  it('should cancel previous requests when loading a new page', async () => {
    ...
    req2.flush([{ id: 16948 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();
  ...
  it('should not fail on resource error state', async () => {
    ...
    expect(comp.urllinks()).toEqual([]);

    // WHEN - second load should still work
    comp.load();
    TestBed.tick();
    const successReq = httpMock.expectOne({ method: 'GET' });
    successReq.flush([{ id: 16948 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();
  ...
  describe('trackId', () => {
    it('should forward to urllinkService', () => {
      const entity = { id: 16948 };
      vitest.spyOn(service, 'getUrllinkIdentifier');
      const id = comp.trackId(entity);
      expect(service.getUrllinkIdentifier).toHaveBeenCalledWith(entity);
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.urllinksParams()).toMatchObject(expect.objectContaining({ sort: ['id,desc'] }));
  });
  ...
  it('should calculate the filter attribute', () => {
    ...
    // THEN
    expect(service.urllinksParams()).toMatchObject(expect.objectContaining({ 'someId.in': ['dc4279ea-cfb9-11ec-9d64-0242ac120002'] }));
  });
