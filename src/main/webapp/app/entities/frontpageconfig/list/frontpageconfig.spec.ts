...
import { sampleWithRequiredData } from '../frontpageconfig.test-samples';
...
describe('Frontpageconfig Management Component', () => {
  ...
  let comp: Frontpageconfig;
  let fixture: ComponentFixture<Frontpageconfig>;
  ...
  beforeEach(() => {
    ...
    fixture = TestBed.createComponent(Frontpageconfig);
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: 17235 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();

    // THEN
    expect(comp.isLoading()).toEqual(false);
    expect(comp.frontpageconfigs()[0]).toEqual(expect.objectContaining({ id: 17235 }));
  });
  ...
  it('should cancel previous requests when loading a new page', async () => {
    ...
    req2.flush([{ id: 17235 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();
  ...
  it('should not fail on resource error state', async () => {
    ...
    expect(comp.frontpageconfigs()).toEqual([]);

    // WHEN - second load should still work
    comp.load();
    TestBed.tick();
    const successReq = httpMock.expectOne({ method: 'GET' });
    successReq.flush([{ id: 17235 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();
  ...
  describe('trackId', () => {
    it('should forward to frontpageconfigService', () => {
      const entity = { id: 17235 };
      vitest.spyOn(service, 'getFrontpageconfigIdentifier');
      const id = comp.trackId(entity);
      expect(service.getFrontpageconfigIdentifier).toHaveBeenCalledWith(entity);
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.frontpageconfigsParams()).toMatchObject(expect.objectContaining({ sort: ['id,desc'] }));
  });
  ...
  it('should calculate the filter attribute', () => {
    ...
    // THEN
    expect(service.frontpageconfigsParams()).toMatchObject(
