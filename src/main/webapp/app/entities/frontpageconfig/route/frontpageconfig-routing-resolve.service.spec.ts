...
import frontpageconfigResolve from './frontpageconfig-routing-resolve.service';
...
describe('Frontpageconfig routing resolve service', () => {
  ...
  describe('resolve', () => {
    it('should return IFrontpageconfig returned by find', async () => {
      // GIVEN
      ...
      await new Promise<void>(resolve => {
        TestBed.runInInjectionContext(() => {
          frontpageconfigResolve(mockActivatedRouteSnapshot).subscribe({
            next(result) {
              // THEN
    ...
    it('should route to 404 page if data not found in server', async () => {
      ...
      await TestBed.runInInjectionContext(async () => {
        await expect(lastValueFrom(frontpageconfigResolve(mockActivatedRouteSnapshot))).rejects.toThrowError('no elements in sequence');
        // THEN
        expect(service.find).toHaveBeenCalledWith(123);
