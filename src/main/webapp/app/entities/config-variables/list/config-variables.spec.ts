...
import { sampleWithRequiredData } from '../config-variables.test-samples';
...
describe('ConfigVariables Management Component', () => {
  ...
  let comp: ConfigVariables;
  let fixture: ComponentFixture<ConfigVariables>;
  ...
  beforeEach(() => {
    ...
    fixture = TestBed.createComponent(ConfigVariables);
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: 13986 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();

    // THEN
    expect(comp.isLoading()).toEqual(false);
    expect(comp.configVariableses()[0]).toEqual(expect.objectContaining({ id: 13986 }));
  });
  ...
  it('should cancel previous requests when loading a new page', async () => {
    ...
    req2.flush([{ id: 13986 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();
  ...
  it('should not fail on resource error state', async () => {
    ...
    expect(comp.configVariableses()).toEqual([]);

    // WHEN - second load should still work
    comp.load();
    TestBed.tick();
    const successReq = httpMock.expectOne({ method: 'GET' });
    successReq.flush([{ id: 13986 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();
  ...
  describe('trackId', () => {
    it('should forward to configVariablesService', () => {
      const entity = { id: 13986 };
      vitest.spyOn(service, 'getConfigVariablesIdentifier');
      const id = comp.trackId(entity);
      expect(service.getConfigVariablesIdentifier).toHaveBeenCalledWith(entity);
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.configVariablesesParams()).toMatchObject(expect.objectContaining({ sort: ['id,desc'] }));
  });
  ...
  it('should calculate the filter attribute', () => {
    ...
    // THEN
    expect(service.configVariablesesParams()).toMatchObject(
