...
import { sampleWithRequiredData } from '../cceleb.test-samples';
import { CcelebService } from '../service/cceleb.service';

import { Cceleb } from './cceleb';
...
describe('Cceleb Management Component', () => {
  ...
  let comp: Cceleb;
  let fixture: ComponentFixture<Cceleb>;
  ...
  beforeEach(() => {
    ...
    fixture = TestBed.createComponent(Cceleb);
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: 27907 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();

    // THEN
    expect(comp.isLoading()).toEqual(false);
    expect(comp.ccelebs()[0]).toEqual(expect.objectContaining({ id: 27907 }));
  });
  ...
  it('should cancel previous requests when loading a new page', async () => {
    ...
    req2.flush([{ id: 27907 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();
  ...
  it('should not fail on resource error state', async () => {
    ...
    expect(comp.ccelebs()).toEqual([]);

    // WHEN - second load should still work
    comp.load();
    TestBed.tick();
    const successReq = httpMock.expectOne({ method: 'GET' });
    successReq.flush([{ id: 27907 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();
  ...
  describe('trackId', () => {
    it('should forward to ccelebService', () => {
      ...
      vitest.spyOn(service, 'getCcelebIdentifier');
      const id = comp.trackId(entity);
      expect(service.getCcelebIdentifier).toHaveBeenCalledWith(entity);
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.ccelebsParams()).toMatchObject(expect.objectContaining({ sort: ['id,desc'] }));
  });
  ...
  it('should calculate the filter attribute', () => {
    ...
    // THEN
    expect(service.ccelebsParams()).toMatchObject(expect.objectContaining({ 'someId.in': ['dc4279ea-cfb9-11ec-9d64-0242ac120002'] }));
  });
