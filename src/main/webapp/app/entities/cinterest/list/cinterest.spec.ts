...
import { sampleWithRequiredData } from '../cinterest.test-samples';
import { CinterestService } from '../service/cinterest.service';

import { Cinterest } from './cinterest';
...
describe('Cinterest Management Component', () => {
  ...
  let comp: Cinterest;
  let fixture: ComponentFixture<Cinterest>;
  ...
  beforeEach(() => {
    ...
    fixture = TestBed.createComponent(Cinterest);
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: 11835 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();

    // THEN
    expect(comp.isLoading()).toEqual(false);
    expect(comp.cinterests()[0]).toEqual(expect.objectContaining({ id: 11835 }));
  });
  ...
  it('should cancel previous requests when loading a new page', async () => {
    ...
    req2.flush([{ id: 11835 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();
  ...
  it('should not fail on resource error state', async () => {
    ...
    expect(comp.cinterests()).toEqual([]);

    // WHEN - second load should still work
    comp.load();
    TestBed.tick();
    const successReq = httpMock.expectOne({ method: 'GET' });
    successReq.flush([{ id: 11835 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();
  ...
  describe('trackId', () => {
    it('should forward to cinterestService', () => {
      ...
      vitest.spyOn(service, 'getCinterestIdentifier');
      const id = comp.trackId(entity);
      expect(service.getCinterestIdentifier).toHaveBeenCalledWith(entity);
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.cinterestsParams()).toMatchObject(expect.objectContaining({ sort: ['id,desc'] }));
  });
  ...
  it('should calculate the filter attribute', () => {
    ...
    // THEN
    expect(service.cinterestsParams()).toMatchObject(expect.objectContaining({ 'someId.in': ['dc4279ea-cfb9-11ec-9d64-0242ac120002'] }));
  });
