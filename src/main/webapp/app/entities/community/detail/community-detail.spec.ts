...
describe('Community Management Detail Component', () => {
      ...
      providers: [
        provideRouter(
          [
            {
              path: '**',
              loadComponent: () => import('./community-detail').then(m => m.CommunityDetail),
              resolve: { community: () => of({ id: 32684 }) },
            },
          ],
  ...
  describe('OnInit', () => {
    it('should load community on init', async () => {
      ...
      const instance = await harness.navigateByUrl('/', CommunityDetail);

      // THEN
      expect(instance.community()).toEqual(expect.objectContaining({ id: 32684 }));
    });
  });
