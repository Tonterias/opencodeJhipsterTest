...
public interface BlogRepository extends JpaRepository<Blog, Long>, JpaSpecificationExecutor<Blog> {
    ...
    @Query(value = "select blog from Blog blog left join fetch blog.community", countQuery = "select count(blog) from Blog blog")
    Page<Blog> findAllWithToOneRelationships(Pageable pageable);

    @Query("select blog from Blog blog left join fetch blog.community")
    List<Blog> findAllWithToOneRelationships();

    @Query("select blog from Blog blog left join fetch blog.community where blog.id =:id")
