...
import com.opencode.test.service.CinterestQueryService;
...
/**
 * REST controller for managing {@link com.opencode.test.domain.Cinterest}.
 */
@RestController
@RequestMapping("/api/cinterests")
public class CinterestResource {

    private static final Logger LOG = LoggerFactory.getLogger(CinterestResource.class);
    ...
    public ResponseEntity<CinterestDTO> createCinterest(@Valid @RequestBody CinterestDTO cinterestDTO) throws URISyntaxException {
        ...
        cinterestDTO = cinterestService.save(cinterestDTO);
        return ResponseEntity.created(new URI("/api/cinterests/" + cinterestDTO.getId()))
            .headers(HeaderUtil.createEntityCreationAlert(applicationName, true, ENTITY_NAME, cinterestDTO.getId().toString()))
            .body(cinterestDTO);
    }
    ...
    public ResponseEntity<CinterestDTO> updateCinterest(
        ...
        cinterestDTO = cinterestService.update(cinterestDTO);
     ...
     */
    @GetMapping("")
    public ResponseEntity<List<CinterestDTO>> getAllCinterests(
        CinterestCriteria criteria,
     ...
     */
    @GetMapping("/{id}")
    public ResponseEntity<CinterestDTO> getCinterest(@PathVariable("id") Long id) {
     ...
     */
    @DeleteMapping("/{id}")
    public ResponseEntity<Void> deleteCinterest(@PathVariable("id") Long id) {
